 ...
 *
 */
public class RadioFieldTag extends TagSupport implements DynamicAttributes{
    ...
    public int doStartTag() throws JspException {
		...
		try{
			StringBuffer builder = new StringBuffer("<input type='radio'");
							...
							if(!StringUtils.isBlank(this.getDicFilter()) ){
								if(dictCodes.contains(dictItem.getCode()) && flag){
									if(!StringUtils.isBlank(this.getValue())&&this.getValue().equals(dictItem.getCode())){
										...
										sb.append("<label class='radio-inline'>").append(builder).append(" value='" + dictItem.getCode() + "'></input>").append(dictItem.getDesc()).append("</label>");
									}
								}
							} else {
								if(!StringUtils.isBlank(this.getValue())&&this.getValue().equals(dictItem.getCode())){
